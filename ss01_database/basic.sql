...
-- cau 5
select a.ma_khach_hang, a.ho_ten, b.ten_loai_khach, c.ma_hop_dong, d.ten_dich_vu, c.ngay_lam_hop_dong, c.ngay_ket_thuc, sum(d.chi_phi_thue + e.so_luong * f.gia) as tong_tien
...
from dich_vu as a join loai_dich_vu as b on b.ma_loai_dich_vu = a.ma_loai_dich_vu
where not exists
(select c.ma_hop_dong from hop_dong as c
where (c.ngay_lam_hop_dong between "2021-01-01" and "2021-03-31") and c.ma_dich_vu = a.ma_dich_vu);
...
where exists
(select c.ma_hop_dong from hop_dong as c 
where year(c.ngay_lam_hop_dong) = "2020" and c.ma_dich_vu = a.ma_dich_vu)
and not exists 
(select c.ma_hop_dong from hop_dong as c 
where year(c.ngay_lam_hop_dong) = "2021" and c.ma_dich_vu = a.ma_dich_vu);
...
-- cau 9
select month(ngay_lam_hop_dong), count(month(ngay_lam_hop_dong)) as so_luong_khach_hang
from hop_dong
...
-- cau 10
select a.ma_hop_dong, a.ngay_lam_hop_dong, a.ngay_ket_thuc, a.tien_dat_coc, sum(b.so_luong) as so_luong_dich_vu_di_kem
from hop_dong a
left join hop_dong_chi_tiet b on a.ma_hop_dong = b.ma_hop_dong
group by a.ma_hop_dong;

-- cau 11
select dich_vu_di_kem.ma_dich_vu_di_kem, ten_dich_vu_di_kem from hop_dong
...
where year(hop_dong.ngay_lam_hop_dong) = '2020' 
    ...
    and month(hop_dong.ngay_lam_hop_dong) <= '12'
...
) as so_hop_dong_nhan_vien on so_hop_dong_nhan_vien.ma_nhan_vien = nhan_vien.ma_nhan_vien
...
where so_hop_dong_nhan_vien.so_hop_dong <=3;
